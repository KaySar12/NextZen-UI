...
<template>
  ...
  >
    ...
    <h1 v-else class="has-text-sub-03 has-text-white mt-6">
    ...
    <span v-else class="has-text-full-03 has-text-grey-600 mt-6"
      >{{ $t("Please") }}
      <a @click="openThirdApp(appDetailData)">{{ $t("Click here") }} </a>
      {{ $t("to open the app. If it does not work, please restart or try again later.") }}
    </span>
    <img
      :src="require('@/assets/img/logo/nextzen.png')"
      alt=""
      class="is-absolute position"
      style="width: 10rem"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async healthCheck() {
      try {
        let res = await this.$openAPI.appManagement.compose.checkComposeAppHealthByID(
          this.appDetailData.name
        );
