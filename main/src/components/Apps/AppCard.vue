			...
			<b-dropdown ref="dro" :mobile-modal="false" :triggers="['contextmenu', 'click']" animation="fade1"
				...
				<b-dropdown-item :focusable="false" aria-role="menu-item" custom>
					...
					<b-button v-if="isV2App && !item.is_uncontrolled" expanded type="is-text"
						@click="checkAppVersion(item.name)">{{ $t("Check then update") }}
...
import domainEditorModal from "@/components/Apps/DomainEditorModal.vue";
...
export default {
	...
	methods: {
		...
		async openSetupDomain(name) {
			try {
				...
				this.$buefy.modal.open({
					...
					animation: "zoom-in",
					props: {
						composeData: YAML.parse(res),
						name,
					},
				});
			} catch {
				console.log("open Setup Domain Error:", e);
			}
		},
		async openTips(name) {
			debugger;
		...
		async rebuild(app) {
			...
			try {
				// 1. get yaml
				const file = await this.$api.container
					.exportAsCompose(app.name)
					.then((res) => res.data);
				// 2. archive
				await this.$api.container.archive(app.name);
				// 3.install compose
				await this.$openAPI.appManagement.compose.installComposeApp(file, {
					name: app.name,
				});
			} catch (e) {
	...
	sockets: {
		...
		"app:install-end"(res) {
			if (res.Properties["dry_run.name"] === this.item.name) {
				...
				// 5.message toast
				this.$buefy.toast.open({
					message: this.$t(`{title} rebulid completed`, {
						title: ice_i18n(this.item.title),
					}),
					type: "is-success",
				});
			}
