        ...
        <div v-if="creatIsShow" class="is-flex-grow-1 is-relative">
          <ValidationObserver ref="ob1">
            ...
            >
              ...
              >
                <b-input
                  v-model="createStorageName"
                  ref="inputs"
                  @keyup.native="
                    createStorageName = createStorageName.replace(/[^\w]/g, '')
                  "
                  @paste.native="
                    createStorageName = createStorageName.replace(/[^\w]/g, '')
                  "
            ...
            <b-field :label="$t('Choose Drive')">
              <b-select v-model="activeDisk" expanded @input="onDiskChoose">
                <option
                  v-for="(option, index) in unDiskData"
                  :key="option.path"
                  :value="index"
                >
            ...
            <section class="message-body">
              <div class="media">
                ...
                <div class="media-content">
                  ...
                  <p class="is-size-14px">
                    {{ $t("The drive you select can be used directly as storage.")
                    }}<br />
                    {{
    ...
    <section v-if="isCreating" class="modal-card-body">
      <div class="installing-warpper mt-6 mb-6">
        <div class="is-flex is-align-items-center is-justify-content-center mb-5">
          <lottie-animation
            :animationData="require('@/assets/ani/creating.json')"
            :autoPlay="true"
            :loop="true"
            class="creating-animation"
          ></lottie-animation>
        </div>
        <h3 class="title is-4 has-text-centered has-text-weight-light">
    ...
    >
      <template v-if="creatIsShow">
        ...
        <div>
          ...
          <b-button
            :label="$t('Format and Create')"
            :loading="isValiding"
            :type="createStorageType == 'format' ? 'is-primary' : ''"
            rounded
            @click="createStorge(true)"
          />
          <b-button
            v-if="createStorageType == 'mountable'"
            :label="$t('Create')"
            :loading="isValiding"
            rounded
            type="is-primary"
  ...
  methods: {
    ...
    async getDiskList(showDefault = false) {
      ...
      try {
        ...
        const storageRes = await this.$api.storage
          .list({ system: "show" })
          .then((v) => v.data.data);
        let storageArray = [];
        ...
        storageRes.forEach((item) => {
          item.children.forEach((part) => {
            ...
            if (
              mergeStorageList.includes(part.uuid) ||
              (mergeStorageList.length > 0 && item.disk_name === "System")
            ) {
        ...
        // sort
        let storageArraySort = orderBy(
          storageArray,
          ["diskName", "label"],
          ["desc", "asc"]
        );
        let mergeConbinationsSort = orderBy(
          mergeConbinations,
          ["diskName", "label"],
          ["desc", "asc"]
        );
