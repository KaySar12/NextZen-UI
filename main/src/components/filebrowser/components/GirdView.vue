<template>
	<div id="select-container" :class="containerHeight" class="scroll-container scrollbars-light is-relative w-full"
		@contextmenu.prevent="openContextMenu" @mousedown.left.prevent="onDragSelectionStart">
		...
		<div class="content-components select-parent">
			<div id="card-container" class="card-container">
				<div v-for="(item, index) in listData" :key="'list-' + index + item.name" :data-rel="index"
					:style="colStyle" class="grid-card rdata selectable">
					<div class="file-card">
		...
		<!-- Context Menu Start -->
		<context-menu ref="ctxMenu" @reload="$emit('reload')" :isLoading="isLoading"
			@updateLoading="handleLoadingUpdate"></context-menu>
		<!-- Context Menu End -->
	</div>
</template>
