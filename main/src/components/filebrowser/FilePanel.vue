							...
							<uploader ref="uploader" :options="options" class="uploader-example">
								...
								<div id="dropTarget" class="is-flex-grow-1 is-flex over is-justify-content-center">
									...
									<component :is="listView" ref="listview" v-model="listData" :isLoading="isLoading"
										...
										@showDetailModal="showDetailModal" @updateLoading="updateLoading"
										@close="handleClose">
...
export default {
	...
	mounted() {
		// Initialize based on the current route's path query
		const initialPath = this.$route.query.path || this.rootPath;
		this.getFileList(initialPath);
	...
	methods: {
		...
		handleDownload() {
			const downItem =
				this.selectedArray.length == 1 ? this.selectedArray[0] : this.selectedArray;
