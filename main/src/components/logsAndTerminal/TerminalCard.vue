<template>
  <fullscreen
    :class="[{ 'mt-4': !fullscreen }, isVaild ? 'fdark-bg' : 'flight-bg']"
    :fullscreen.sync="fullscreen"
    :page-only="true"
    :teleport="true"
    class="fullScreen pl-2 pt-2 pb-2"
    @change="onWindowResize"
  >
    ...
    <div id="terminal" class="is-flex is-align-items-center is-justify-content-center">
      ...
      <div
        v-else
        id="xterm"
        :class="[fullscreen ? 'fullheight' : 'sheight']"
        class="xterm"
      ></div>
    </div>
  </fullscreen>
  ...
  methods: {
    async checkLogin() {
      ...
      try {
        ...
        this.wsUrl = `${this.$wsProtocol}//${this.$baseURL}/v1/sys/wsssh?${qs.stringify(
          postData
        )}`;
        this.initSocket();
      } catch (error) {
